...
void my_usb_init(int ven, int pro) {
...
int main(int argc, char **argv) {
    int command = STATUS;
    if(argc == 2) {
        if(!strcmp(argv[1], "on")) command = ON;
        else if(!strcmp(argv[1], "off")) command = OFF;
        else if(!strcmp(argv[1], "status")) command = STATUS;
        else if(!strcmp(argv[1], "toggle")) command = TOGGLE;
        else fatal("Unrecognized argument: %s\n", argv[1]);
    } else if(argc > 1) {
        fatal("Too many arguments\n");
    }

    my_usb_init(0x067b, 0x2303);
